...
export default function ModalVisualizarTarefa({
  ...
  const [tarefaComItem, setTarefaComItem] = useState<TarefaComItem | null>(
    null,
  );
  ...
  const [nomeResponsavelAtual, setNomeResponsavelAtual] =
  ...
  const handleSalvarResponsavel = async () => {
    ...
    try {
      ...
      await ApiTarefas.put(`/tarefas/tarefas/${tarefa.id}`, tarefaAtualizada);

      const novoResponsavel = responsaveis.find(
        (r) => r.id === Number(responsavelSelecionado),
      );
  ...
  const handleAbrirEdicao = () => {
    if (!podeGerenciarTodasTarefas) return;
    setResponsavelSelecionado(
      tarefa?.responsavelId ? String(tarefa.responsavelId) : "",
    );
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center z-[60]">
      ...
      <div
        className="relative p-6 rounded-lg shadow-2xl w-full max-w-lg z-10 border border-[#3e3e3e]"
      ...
      >
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            {!editandoResponsavel ? (
              <div
                className={`flex items-center justify-between bg-[#1f1f1f] p-3 rounded border border-[#3e3e3e] transition-colors ${
                  podeGerenciarTodasTarefas
                    ? "cursor-pointer hover:border-blue-500"
                    : ""
                }`}
                onClick={
                  podeGerenciarTodasTarefas ? handleAbrirEdicao : undefined
                }
              >
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <p className="text-white mt-1">
                {tarefa.tempoMaximoMinutos} minutos
              </p>
            </div>
            <div>
              <label className="text-xs font-bold text-gray-500 uppercase">
              ...
              {carregando ? (
                ...
                <div className="p-3 rounded bg-[#2a2a2a] border-l-4 border-blue-500 shadow-sm">
                  <div className="text-white text-sm font-semibold">
                    {tarefaComItem.item.nome}
                  </div>
                  <div className="text-gray-400 text-xs">
                    {tarefaComItem.item.descricao}
                  </div>
                </div>
              ) : (
                    ...
                    <button
                      onClick={() => {
                        ...
                        window.dispatchEvent(
                          new CustomEvent("abrirModalItem", {
                            detail: { tarefaId: tarefa.id },
                          }),
                        );
                      }}
