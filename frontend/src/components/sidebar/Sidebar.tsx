        ...
        <nav className="flex flex-1 flex-col gap-1 overflow-y-auto px-2 py-4">
          {visibleNavItems.map(({ to, icon: Icon, label }) => (
            <NavLink
              ...
              className={({ isActive }) => `flex items-center gap-3 rounded-lg px-2 py-2.5 transition-colors duration-150 ${
                isActive
            ...
            {expanded && (
              <>
                ...
                <button type="button" onClick={() => setConfirmLogout(true)} aria-label="Sair">
                  <LogOut size={16} className="text-white/40 transition-colors hover:text-red-400" />
                </button>
              </>
            )}
          </div>
