...
import {
  carregarProfissionaisPorProjeto,
  carregarProfissionalPorId,
} from "../../service/servicoProfissionais"
...
function DetalhesProjeto() {
    ...
    async function load(id: number) {
      ...
      try {
        ...
        setItens(
          podeGerenciarProjetos
            ? itensData
            : itensData.filter((item) => itemIdsPermitidos.has(Number(item.idItem))),
        )
      } catch (error) {
  ...
  const getNomeProfissional = (id?: number | null) => {
    ...
    return (
      profissionais.find((profissional) => Number(profissional.id) === Number(id))?.nome ??
      `ID ${id}`
    )
  }
  ...
  }, [tarefas, pesquisaTarefa, filtroTipo, filtroItem, filtroProfissional, profissionais, itens])
  ...
  const metricas = useMemo(() => {
    ...
    const emAndamento = tarefas.filter((tarefa) => tarefa.status === "EM_ANDAMENTO").length
    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-[#1f1f1f] p-6 text-white">
        <div className="max-w-md rounded-2xl border border-red-500/20 bg-red-500/10 p-6 text-center">
          ...
          <p className="mt-2 text-sm text-red-100/80">Você não está vinculado a este projeto.</p>
          <button
            onClick={() => navigate("/projetos")}
        ...
        <section className="overflow-hidden rounded-[20px] border border-white/10 bg-[#232329] shadow-[0_18px_60px_rgba(0,0,0,0.25)]">
          ...
          <div className="grid gap-4 px-6 py-5 sm:grid-cols-2 lg:grid-cols-4 sm:px-8">
            <div className="rounded-2xl border border-white/10 bg-[#2c2c31] p-4">
              ...
              <p className="mt-2 text-2xl font-bold text-white">{metricas.totalTarefas}</p>
              <p className="mt-1 text-xs text-slate-500">{metricas.concluidas} concluída(s)</p>
            </div>

            <div className="rounded-2xl border border-white/10 bg-[#2c2c31] p-4">
              <p className="text-xs text-slate-500">Em andamento</p>
              <p className="mt-2 text-2xl font-bold text-white">{metricas.emAndamento}</p>
              <p className="mt-1 text-xs text-slate-500">{metricas.pendentes} pendente(s)</p>
            </div>
            ...
            <div className="rounded-2xl border border-white/10 bg-[#2c2c31] p-4">
              ...
              <p className="mt-1 text-xs text-slate-500">Soma do tempo das tarefas</p>
            </div>

            <div className="rounded-2xl border border-white/10 bg-[#2c2c31] p-4">
              <p className="text-xs text-slate-500">Equipe</p>
              <p className="mt-2 text-2xl font-bold text-white">{profissionais.length}</p>
              <p className="mt-1 text-xs text-slate-500">Profissional(is) vinculado(s)</p>
            </div>
          </div>
        </section>
          ...
          <main className="flex min-w-0 flex-col gap-5">
            <section className="rounded-[20px] border border-white/10 bg-[#232329] p-5 shadow-lg shadow-black/10">
              <div className="mb-5 flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                <div>
                  ...
                  <h2 className="mt-2 text-2xl font-bold text-white">Informações principais</h2>
                </div>
            ...
            <section className="rounded-[20px] border border-white/10 bg-[#232329] p-5 shadow-lg shadow-black/10">
              ...
              <div className="mb-5 grid gap-3 md:grid-cols-2 xl:grid-cols-4">
                <select
                  value={filtroItem ?? ""}
                  onChange={(e) => setFiltroItem(e.target.value ? Number(e.target.value) : null)}
                  className="h-11 rounded-2xl border border-white/10 bg-[#1a1a20] px-4 text-sm text-white outline-none transition focus:border-white/20"
                >
                  <option value="">
                ...
                <select
                  ...
                  onChange={(e) =>
                    setFiltroProfissional(e.target.value ? Number(e.target.value) : null)
                  }
                  className="h-11 rounded-2xl border border-white/10 bg-[#1a1a20] px-4 text-sm text-white outline-none transition focus:border-white/20"
                >
          ...
          <aside className="flex min-w-0 flex-col gap-5">
            <section className="rounded-[20px] border border-white/10 bg-[#232329] p-5 shadow-lg shadow-black/10">
              <div className="mb-5">
                ...
                <h2 className="mt-2 text-2xl font-bold text-white">Distribuição</h2>
              </div>

              <div className="space-y-3">
                {[
                  { label: "Pendente", value: metricas.pendentes, status: "PENDENTE" },
                  { label: "Em andamento", value: metricas.emAndamento, status: "EM_ANDAMENTO" },
                    ...
                    <div key={item.status} className="rounded-2xl bg-[#1f1f24] p-4">
                      <div className="mb-3 flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          <span
                            className={`h-2.5 w-2.5 rounded-full ${getStatusDot(item.status)}`}
                          />
                      ...
                      <div className="h-2 overflow-hidden rounded-full bg-black/30">
                        <div
                          className={`h-full rounded-full ${getStatusDot(item.status)}`}
            ...
            <section className="rounded-[20px] border border-white/10 bg-[#232329] p-5 shadow-lg shadow-black/10">
              <div className="mb-5 flex items-center justify-between gap-4">
                <div>
                  ...
                  <h2 className="mt-2 text-2xl font-bold text-white">Profissionais</h2>
                </div>
                ...
                {profissionaisFiltrados.length > 0 ? (
                  profissionaisFiltrados.map((profissional) => {
                    const tarefasDoProfissional = tarefas.filter(
                      (tarefa) => Number(tarefa.responsavelId) === Number(profissional.id),
                    ).length
