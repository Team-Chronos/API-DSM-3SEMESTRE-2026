  ...
  return (
    <>
      <div className="relative">
        <button className="absolute cursor-pointer top-4 right-4" onClick={handleFechar}>
          ✕
        </button>
        <div className="flex flex-col gap-y-3 p-4">
          <h2 className="text-lg font-medium pr-6">{tarefa.titulo}</h2>
          <div className="relative flex flex-col lg:flex-row gap-8">
            <div className="flex flex-col grow gap-y-4">
              {tarefa.descricao && <p className="text-sm">{tarefa.descricao}</p>}
            ...
            <div className="max-w-full lg:max-w-4/12 min-w-60 grow flex flex-col gap-y-4 lg:mr-4">
              <div className="flex flex-col bg-mist-700 rounded-lg p-3">
                ...
                <div className="w-full bg-mist-800 h-3 rounded-full">
                  <div
                    className="h-3 max-w-full bg-violet-700 shadow-violet-800 shadow-inner rounded-full"
                    style={{ width: `${porcentagemTempo * 100}%` }}
                  ></div>
                </div>
                <p className="mt-2 text-white/90 text-sm">
                  {registroHorasTarefa?.tempoMinutos || 0} min / {tarefa.tempoMaximoMinutos} min
                </p>
                {porcentagemTempo === 1 ? (
                  <>
                    <button
                    ...
                    <p className="mt-3 text-white/50 text-xs text-center">
                      *Tempo máximo atingido. Fale com o gerente do projeto*
                    </p>
                  </>
                ) : (
                  <button
