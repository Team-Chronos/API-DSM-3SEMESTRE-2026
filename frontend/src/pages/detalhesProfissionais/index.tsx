...
import { type Projeto, type Profissional, getInitials } from "../listaProfissionais";


const CARGOS = [
  { id: 1, label: "Dev", color: "bg-[#2d1f6e] text-[#a78bfa] border border-[#6627cc]/40" },
  { id: 2, label: "Gerente", color: "bg-[#1a2e1a] text-[#4ade80] border border-[#22c55e]/30" },
  { id: 3, label: "Financeiro", color: "bg-[#1e2a1e] text-[#86efac] border border-[#16a34a]/30" },
];
...
function TelaDetalhesProfissional() {
  ...
  const [form, setForm] = useState({ nome: "", email: "", cargoId: 1, ativo: true });
  ...
  useEffect(() => {
    setLoading(true);
    fetch(`http://localhost:8081/api/profissionais/${id}`)
      .then((res) => {
      ...
      .then((data: Profissional) => {
        ...
        setForm({ nome: data.nome, email: data.email, cargoId: data.cargoId, ativo: data.ativo });
        setLoading(false);
      })
      .catch((err) => {
        setErro(err.message);
  ...
  function handleCancelarEdicao() {
    ...
    setForm({ nome: profissional.nome, email: profissional.email, cargoId: profissional.cargoId, ativo: profissional.ativo });
    setEditando(false);
    setFeedback(null);
  }
  ...
  async function handleSalvar() {
    ...
    setFeedback(null);
    try {
      const res = await fetch(`http://localhost:8081/api/profissionais/${id}`, {
        method: "PUT",
      ...
      setForm({ nome: atualizado.nome, email: atualizado.email, cargoId: atualizado.cargoId, ativo: atualizado.ativo });
      setEditando(false);
      setFeedback({ tipo: "sucesso", mensagem: "Profissional atualizado com sucesso!" });
    } catch (err: any) {
  ...
  async function handleExcluir() {
    ...
    setExcluindo(true);
    try {
      const res = await fetch(`http://localhost:8081/api/profissionais/${profissional.id}`, {
        method: "DELETE",
      });
  ...
  const cargoAtual = CARGOS.find((c) => c.id === (editando ? form.cargoId : profissional?.cargoId));
  ...
  if (erro || !profissional) {
    return (
      <div className="flex h-96 flex-col items-center justify-center gap-4 text-slate-400">
        ...
        <button onClick={() => navigate("/profissionais")} className="text-sm text-[#9d71f5] hover:text-white transition">
          ← Voltar para a lista
        </button>
      </div>
    );
              ...
              <div className="relative flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <div className="mt-1 flex items-center gap-2">
                      {cargoAtual && (
                        <span className={`inline-block rounded-lg px-2.5 py-0.5 text-xs font-semibold ${cargoAtual.color}`}>
                          {cargoAtual.label}
                        </span>
                      )}
                      <span className={`inline-flex items-center gap-1 rounded-lg px-2.5 py-0.5 text-xs font-semibold ${
                        ...
                        <span className={`h-1.5 w-1.5 rounded-full ${profissional.ativo ? "bg-green-400" : "bg-red-400"}`} />
                ...
                <div className="flex gap-2">
                  {!editando ? (
                    <>
                      <button
                        onClick={() => { setEditando(true); setFeedback(null); }}
              ...
              <div className="grid gap-5 sm:grid-cols-2">
                ...
                <div className="space-y-2">
                  ...
                  {editando ? (
                    <select
                      value={form.cargoId}
                      onChange={(e) => setForm((f) => ({ ...f, cargoId: Number(e.target.value) }))}
                    ...
                    <div className="rounded-xl border border-white/5 bg-[#1a1a20] px-4 py-2.5">
                      {cargoAtual && (
                        <span className={`inline-block rounded-lg px-3 py-1 text-xs font-semibold ${cargoAtual.color}`}>
                          {cargoAtual.label}
                        </span>
                      )}
                    </div>
                  ...
                  {editando ? (
                    <div className="flex items-center gap-3 rounded-xl border border-white/10 bg-[#1a1a20] px-4 py-2.5">
                      <button
                        ...
                        className={`relative h-5 w-9 rounded-full transition-colors duration-200 ${form.ativo ? "bg-[#6627cc]" : "bg-slate-600"}`}
                      >
                        <span className={`absolute left-0.5 top-0.5 h-4 w-4 rounded-full bg-white shadow transition-transform duration-200 ${form.ativo ? "translate-x-4" : "translate-x-0.5"}`} />
                      </button>
                      <span className="text-sm text-slate-300">{form.ativo ? "Ativo" : "Inativo"}</span>
                    </div>
                  ) : (
                    <div className="rounded-xl border border-white/5 bg-[#1a1a20] px-4 py-2.5">
                  ...
                  {profissional.projetos.map((proj) => (
                    ...
                    >
                      <div className="min-w-0">
                        <p className="truncate text-sm font-medium text-white">{proj.nomeProjeto}</p>
                        <p className="mt-0.5 text-xs text-slate-500">{proj.codigoProjeto}</p>
                      </div>
